...
async function deploy() {
	...
	console.log('Deployed Address: ', deployedAddress.bech32());

	// update deployed SC address in config.ts
	const configFilePath = 'src/config.ts';
	const content = fs.readFileSync(configFilePath, 'utf8');
	const re = new RegExp('XRAFFLE_SC_ADDRESS = \'[a-zA-Z0-9]+\'', 'gm');
	const formatted = content.replace(re, `XRAFFLE_SC_ADDRESS = \'${deployedAddress.bech32()}\'`);
	
	fs.writeFileSync(configFilePath, formatted, 'utf8');
}
